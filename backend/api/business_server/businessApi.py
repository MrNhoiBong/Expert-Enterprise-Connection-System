...
def upload_file(body: FileUploadRequest,
                current_user: dict = Depends(get_current_user),
                factory: ServiceFactory = Depends(get_factory)):
    ...
    # Kiểm tra expert có trong project không
    participation = factory.dao.search(
        "expertParticipate",
        {"expertID": current_user["userId"], "projectID": body.project_id}
    )
    ...
    return {"message": "Upload thành công", "docID": doc_id}
