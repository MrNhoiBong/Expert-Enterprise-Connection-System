...
export default function ExpertContacts() {
    const { profile } = useAuth()
    const myId = profile?.expertID || profile?.enterpriseID || profile?.FoundID
    const bottomRef = useRef(null)

    const [contacts, setContacts] = useState([])   // all contact docs
    const [active, setActive] = useState(null)  // selected contactID
    ...
    // ── Load contacts (+ poll mỗi 5s để nhận request mới) ──
    useEffect(() => {
        let mounted = true
    ...
    // ── Poll messages mỗi 2s khi đang mở chat ──
    useEffect(() => {
        if (!active) return
        let mounted = true
        const timer = setInterval(() => {
            bizApi.getMessages(activeRef.current).then(d => {
                ...
                // Chỉ update nếu có tin nhắn mới (tránh re-render không cần thiết)
                setMessages(prev => {
    ...
    // ── Get peer name from a contact doc ──
    ...
    // ── Load peer info for display ──
    // Enrich contacts with peer name by fetching from DB
    useEffect(() => {
        ...
        contacts.forEach(async c => {
            ...
            if (c.receiverName || c.senderName) return // already enriched
            try {
                let peer
            ...
            <div style={{ display: 'flex', height: 'calc(100vh - 64px)', overflow: 'hidden', margin: '-32px -32px 0', fontFamily: 'Inter,sans-serif' }}>
                ...
                <aside style={{ width: 300, background: T.surfaceContainerLow, display: 'flex', flexDirection: 'column', borderRight: `1px solid ${T.outlineVariant}30`, flexShrink: 0 }}>
                    ...
                    <div className="msg-scroll" style={{ flex: 1, overflowY: 'auto', padding: '8px 10px' }}>
                        ...
                        {tab === 'chats' && (
                            loading ? <div style={{ textAlign: 'center', padding: 20, color: T.outline, fontSize: 12 }}>Loading...</div>
                                : accepted.length === 0
                                    ? <div style={{ textAlign: 'center', padding: 32, color: T.outline }}>
                                        ...
                                        <p style={{ fontSize: 11, marginTop: 4 }}>Go to Discovery to connect.</p>
                                    </div>
                                    : accepted.map(c => {
                                        ...
                                        {pending.map(c => (
                                            <div key={c.contactID} style={{ background: 'white', borderRadius: 12, padding: 16, marginBottom: 10, border: `1px solid ${T.outlineVariant}30` }}>
                                                ...
                                                <div style={{ display: 'flex', gap: 8 }}>
                                                    ...
                                                    <button className="btn btn-secondary" style={{ flex: 1, justifyContent: 'center', fontSize: 12, color: T.error, borderColor: T.errorContainer, background: T.errorContainer }}
                    ...
                    {!active ? (
                        ...
                        <>
                            ...
                            <div className="msg-scroll" style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                                {messages.length === 0 && (
                                    <div style={{ textAlign: 'center', color: T.outline, fontSize: 13, marginTop: 40 }}>
